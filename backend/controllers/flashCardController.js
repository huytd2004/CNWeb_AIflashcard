...
const {
  ...
  determineCardStatus,
  calculatePercentage,
} = require("../services/SM2_Algorithm");
...
const setCache = async (key, data, ttl = 3600) => {
  ...
  await CacheModel.updateOne(
    { key },
    { data: JSON.parse(JSON.stringify(data)), expireAt },
    { upsert: true }
  );
};
...
exports.translateAIEnhance = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ ok: true, message: "Dịch thuật thành công", parse });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
      .json({ message: "Lỗi khi tạo flashcard", error: error.message });
  }
};
  ...
  try {
    ...
    if (listFlashCard.userId.equals(id) === false) {
      return res.status(403).json({
        message: "Bạn không có quyền tạo flashcard trong danh sách này",
      });
    }
    const result = await model.generateContent(prompt);
...
exports.createFlashCard = async (req, res) => {
  try {
    ...
    if (!title) {
      return res
        .status(400)
        .json({ message: "Thiếu thông tin bắt buộc (tên từ, định nghĩa)" });
    }
  ...
  try {
    ...
    if (!Array.isArray(cards) || cards.length === 0) {
      return res.status(400).json({
        message: "Vui lòng cung cấp một mảng thẻ hợp lệ để cập nhật.",
      });
    }
    ...
    for (const cardUpdate of cards) {
      ...
      if (
        ...
        errors.push({
          id,
          message:
            "Thiếu thông tin (id, quality, userId) hoặc userId không hợp lệ.",
        });
        continue;
      }

      const currentCard = await FlashCard.findOne({
        _id: id,
        userId: userId,
      }).lean();
      ...
      if (!currentCard) {
        errors.push({
          id,
          message: `Không tìm thấy flashcard với ID: ${id} hoặc bạn không có quyền truy cập.`,
        });
        continue;
      }

      const { efactor, interval, repetitions } = SM2_Algorithm(
        currentCard.efactor,
    ...
    if (errors.length > 0) {
      res.status(200).json({
        message:
          "Đã cập nhật một phần các flashcard, nhưng có lỗi xảy ra với một số thẻ.",
        errors: errors,
      });
    } else {
      res.status(200).json({
        ok: true,
        message: "Tất cả flashcard đã được cập nhật thành công.",
      });
    }
  } catch (error) {
    console.error("Lỗi khi cập nhật flashcards hàng loạt:", error);
    res.status(500).json({
      message: "Lỗi server khi cập nhật flashcards hàng loạt.",
      error: error.message,
    });
  }
};
  ...
  try {
    ...
    if (!listFlashCard) {
      return res.status(404).json({
        message: "Tìm không thấy flashcard này, vui lòng f5 lại trang",
      });
    }
    ...
    for (const flashcardData of data) {
      ...
      if (!title || !define) {
        return res
          .status(400)
          .json({ message: "flashcard cần có title và define" });
      }
    ...
    return res
      .status(500)
      .json({ message: "Lỗi khi tạo flashcards", error: error.message });
  }
};
...
exports.getFlashCardById = async (req, res) => {
  try {
    ...
    if (!listFlashCards) {
      return res.status(404).json({
        message: "Không tìm thấy danh sách flashcards cho người dùng này",
      });
    }
    ...
    return res.status(500).json({
      message: "Lỗi khi lấy danh sách flashcards",
      error: error.message,
    });
  }
};
...
exports.getFlashCardByIdToPractive = async (req, res) => {
  try {
    ...
    const list_flashcard = await ListFlashCard.findById(fc_id).populate(
      "flashcards"
    );
    if (!list_flashcard) {
    ...
    const cardsDueToday = list_flashcard.flashcards.filter(
...
exports.getFlashCardToPractive = async (req, res) => {
  try {
    ...
    const cardsDueToday = fetchedCards.filter(
      (card) => new Date(card.nextReviewDate) <= today
    );
    cardsDueToday.sort(
      (a, b) =>
...
exports.updateFlashCard = async (req, res) => {
  try {
    ...
    if (!flashcard) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy flashcard này để cập nhật" });
    }

    return res
      .status(200)
      .json({ ok: true, message: "Flashcard đã được cập nhật", flashcard });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Lỗi khi cập nhật flashcard", error: error.message });
  }
};
...
exports.deleteFlashCard = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ok: true,
      message: `Từ ${flashcard.title} đã được xóa thành công`,
    });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "Lỗi khi xóa flashcard", error: error.message });
  }
};
...
exports.createListFlashCard = async (req, res) => {
  try {
    ...
    const result = await ListFlashCard.findById(newListFlashCard._id)
      .populate("flashcards")
      .populate("userId", "_id displayName profilePicture");
    await handleCreateActivity(
      id,
      ...
      newListFlashCard._id.toString()
    );
    return res.status(201).json({
      ok: true,
      message: "Danh sách flashcards đã được tạo thành công",
      listFlashCard: result,
    });
  } catch (error) {
    console.error(error);
    return res.status(500).json({
      message: "Lỗi khi tạo danh sách flashcards",
      error: error.message,
    });
  }
};
    ...
    listFlashCards.forEach((list) => {
      ...
      list.flashcards.forEach((card) => {
        if (card.history && card.history.length > 0) {
          ...
          totalCorrectReviews += card.history.filter(
            (h) => h.quality >= 3
          ).length;
        }
      ...
      list.accuracyPercentage =
        totalAllReviews === 0
          ? 0
          : Math.round((totalCorrectReviews / totalAllReviews) * 100);
...
exports.getAllListFlashCardsWithExtension = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ ok: false, message: "Người dùng không tìm thấy" });
    }
    const listFlashCards = await ListFlashCard.find({ userId: id })
      .sort({ created_at: -1 })
...
exports.getListFlashCardById = async (req, res) => {
  try {
    ...
    const listFlashCard = await ListFlashCard.findById(id).populate(
      "flashcards"
    );
    ...
    if (!listFlashCard) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy danh sách flashcards này" });
    }
...
exports.updateListFlashCard = async (req, res) => {
  try {
    ...
    const listFlashCard = await ListFlashCard.findByIdAndUpdate(
      _id,
      updateData,
      { new: true }
    );
    ...
    if (!listFlashCard) {
      return res.status(404).json({
        message: "Không tìm thấy danh sách flashcards này để cập nhật",
      });
    }
    ...
    return res.status(200).json({
      ok: true,
      message: "Danh sách flashcards đã được cập nhật",
      listFlashCard,
    });
  } catch (error) {
    return res.status(500).json({
      message: "Lỗi khi cập nhật danh sách flashcards",
      error: error.message,
    });
  }
};
...
exports.deleteListFlashCard = async (req, res) => {
  try {
    ...
    if (!listToDelete) {
      return res.status(404).json({
        message:
          "Không tìm thấy danh sách flashcard để xóa hoặc bạn không có quyền.",
      });
    }
    ...
    const deleteListResult = await ListFlashCard.deleteOne({
      _id: _id,
      userId: id,
    });
    ...
    if (deleteListResult.deletedCount === 0) {
      ...
      return res
        .status(404)
        .json({ message: "Lỗi không xác định khi xóa danh sách." });
    }
    ...
    return res.status(200).json({
      ok: true,
      message:
        "Danh sách flashcard và các thẻ liên quan đã được xóa thành công.",
      deletedListId: _id,
      deletedCardsCount: cardIdsToDelete ? cardIdsToDelete.length : 0,
    });
  } catch (error) {
    return res.status(500).json({
      message: "Lỗi khi xóa danh sách flashcards",
      error: error.message,
    });
  }
};
...
exports.getAllFlashCards = async (req, res) => {
  try {
    ...
    if (cachedData) {
      return res
        .status(200)
        .json({ ok: true, publicFlashcards: cachedData.data });
    }
    const publicFlashcards = await ListFlashCard.find()
      .populate("userId", "_id displayName profilePicture")
      .sort({ created_at: -1 })
      .exec();
...
exports.statisticsSumarry = async (req, res) => {
  try {
    ...
    if (!userId || userId === "default_user_id") {
      return res
        .status(400)
        .json({ message: "Vui lòng cung cấp User ID hợp lệ." });
    }
    ...
    allFlashcards.forEach((card) => {
      const wordData = {
        ...
        title: card.title,
        define: card.define,
        transcription: card.transcriptin,
    ...
    allFlashcards.forEach((card) => {
      if (
        card.history &&
        card.history.some((h) => new Date(h.date) >= oneWeekAgo)
      ) {
    ...
    await setCache(`summary_${userId}`, {
      weeklyReviewedWordsCount,
      wordAccuracy,
      learnedWords,
    });

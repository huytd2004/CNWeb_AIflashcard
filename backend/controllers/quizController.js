  ...
  try {
    ...
    const [quiz, total] = await Promise.all([
      QuizModel.find(query)
        .populate("uid", "profilePicture displayName verify")
        .skip(skip)
        .limit(limit)
        .sort({ date: -1 })
        .lean(),
...
const getQuizByUser = async (req, res) => {
  try {
    ...
    if (search) {
      ...
      query.uid = id; // Đảm bảo vẫn filter theo user
    }

    const quiz = await QuizModel.find(query)
      .populate("uid", "_id displayName profilePicture")
      .sort({ date: -1 });
...
const getQuizBySubject = async (req, res) => {
  try {
    const { id } = req.params;
    const quiz = await QuizModel.find({ subject: id, status: true })
      .populate("uid", "displayName profilePicture verify")
      .sort({ date: -1 });
    console.error(quiz);
    if (!quiz) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy Quiz", ok: false });
    }
...
const getQuizAdmin = async (req, res) => {
  try {
    const quiz = await QuizModel.find()
      .populate("uid", "_id displayName profilePicture created_at")
      .sort({ date: -1 })
      .lean()
      .exec();
...
const getQuizById = async (req, res) => {
  try {
    ...
    if (!quiz) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy Quiz", status: 404 });
    }
...
const CreateComment = async (req, res) => {
  try {
    ...
    const existingCommentIndex = quiz.comment.findIndex(
      (cmt) => cmt.user_id.toString() === id
    );
    ...
    if (existingCommentIndex !== -1) {
      ...
      res
        ...
        .json({
          ok: true,
          exist: true,
          id: quiz.comment[existingCommentIndex]._id,
  ...
  try {
    ...
    const invalidQuestionIndex = questions.findIndex((question, index) => {
      if (!question.question) {
        return res
          .status(400)
          .json({ message: `Câu hỏi đang bị để trống ở câu ${index + 1}` });
      }
      if (!Array.isArray(question.answers) || question.answers.length === 0) {
        return res
          .status(400)
          .json({ message: `Đáp án câu hỏi số ${index + 1} chưa có` });
      }
      if (question.correct == -1) {
        return res
          .status(400)
          .json({
            message: `Đáp án câu ${index + 1} bạn chưa chọn hoặc không hợp lệ`,
          });
      }
      return false;
    });
    ...
    const newQuiz = new QuizModel({
      ...
      img:
        img ||
        "https://art.vinadesign.vn/img/khong-co-ap-luc-khong-co-kim-cuong-design-1-mockup-vector.jpg",
    ...
    res
      .status(201)
      .json({ ok: true, message: "Tạo Quiz thành công", quiz: savedQuiz });
  } catch (error) {
    console.error(error);
    res
      .status(500)
  ...
  try {
    ...
    if (!quizId) {
      return res
        .status(400)
        .json({
          message: "Bạn chưa đăng nhập, vui lòng reload lại trang",
          status: 400,
        });
    }
    ...
    if (req.body.title !== undefined) {
      ...
      updateFields.slug =
        slugify(req.body.title, { lower: true }) + "-" + generateRandomSlug();
    }
    ...
    if (!updatedQuiz) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy quiz để cập nhật", status: 404 });
    }
    ...
    res
      .status(200)
      .json({ ok: true, message: "Cập nhật Quiz thành công", updatedQuiz });
  } catch (error) {
    console.error(error);
    res
      .status(500)
...
const updateAllQuiz = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ ok: true, message: "Cập nhật Quiz thành công", updateQuiz });
  } catch (error) {
    console.error(error);
  ...
  try {
    ...
    if (status) {
      const newNotification = new Notification({
        ...
        link: `/quiz/${quiz.slug}`,
        content:
          "Bài quiz của bạn đã được xét duyệt và được hiển thị lên trang chủ.",
      });
      ...
      const newNotification = new Notification({
        ...
        link: `/quiz/${quiz.slug}`,
        content:
          "Bài quiz của bạn đã bị ẩn khỏi trang chủ do vi phạm quy định của chúng tôi.",
      });
...
const increaseHelpfulCount = async (req, res) => {
  try {
    ...
    if (comment.helpful.includes(id)) {
      return res
        .status(400)
        .json({ message: "Bạn đã bầu chọn cho bình luận này rồi" });
    }
...
const updateQuizNew = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ ok: true, message: "Cập nhật Quiz thành công", findQuestion });
  } catch (error) {
    console.error("DocumentBank Error:", error);
    res.status(500).json({
      message: "Server gặp lỗi, vui lòng thử lại sau ít phút",

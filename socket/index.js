...
console.log("Socket Service ENV:", process.env.NODE_ENV);
console.log("Connecting to Backend at:", process.env.API_ENDPOINT);
...
io.on("connection", (socket) => {
  ...
  socket.on("sendMessageCommu", async (data) => {
    ...
    try {
      const chat = await axios.post(
        process.env.API_ENDPOINT + "/chatcommu",
        { userId, message, image, replyTo },
  ...
  socket.on("unsendMessageCommu", async (data) => {
    ...
    try {
      const chat = await axios.post(
        process.env.API_ENDPOINT + "/chatcommu/unsend",
        { userId, messageId },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  socket.on("reactMessagePrivate", async (data) => {
    ...
    try {
      ...
      if (response.data.ok) {
        io.to(chatRoomId).emit("replyReactMessage", {
          messageId,

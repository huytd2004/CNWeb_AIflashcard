document.addEventListener("DOMContentLoaded", function () {
  ...
  if (btnDanger) {
    btnDanger.addEventListener("click", function () {
      window.open("https://hustvanvo.io.vn/auth/login", "_blank");
  ...
  if (profile) {
    chrome.storage.local.get(
      ["profile", "listFlashCards", "list_flashcard_id"],
      function (result) {
        if (result.profile) {
          ...
          if (result.listFlashCards && result.listFlashCards.length > 0) {
            ...
            showNotification(
              "Không có flashcard nào được lưu trữ, bạn chưa đăng nhập"
            );
          }
        } else {
  ...
  if (selectFlashcard) {
    ...
    selectFlashcard.addEventListener("change", function (e) {
      chrome.storage.local.get(["listFlashCards"], function (result) {
        ...
        var findListFC = result.listFlashCards.find(
          (item) => item._id === selectedId
        );

        // Lưu với error handling và callback
        chrome.storage.local.set(
          { list_flashcard_id: findListFC },
          function () {
            if (chrome.runtime.lastError) {
              console.error(
                "Error saving flashcard selection:",
                chrome.runtime.lastError
              );
              showNotification("Lỗi khi lưu lựa chọn flashcard!");
            } else {
              console.log(
                "Successfully saved flashcard selection:",
                findListFC
              );
            }
          }
    ...
    chrome.storage.local.get(["target_language"], function (result) {
      ...
      if (result.hasOwnProperty("target_language") && result.target_language) {
        ...
        chrome.storage.local.set({ target_language: "vi" }, function () {
          if (chrome.runtime.lastError) {
            console.error(
              "Error saving default target_language:",
              chrome.runtime.lastError
            );
          }
        });
  ...
  chrome.storage.local.get(["listFlashCards"], function (result) {
    ...
    if (flashcardList && flashcardList.length > 0) {
      ...
      chrome.storage.local.get(["list_flashcard_id"], function (result) {
        if (chrome.runtime.lastError) {
          console.error(
            "Error reading list_flashcard_id:",
            chrome.runtime.lastError
          );
          // Set default anyway
        ...
        if (result.list_flashcard_id) {
          ...
          const existingFlashcard = flashcardList.find(
            (item) => item._id === result.list_flashcard_id._id
          );
          ...
          if (existingFlashcard) {
            ...
            selectFlashcard.value = result.list_flashcard_id._id;
            console.log(
              "Set selected flashcard from storage:",
              result.list_flashcard_id.title
            );
          } else {
      ...
      selectFlashcard.innerHTML =
        "<option value=''>No flashcards available</option>";
    }
  });
}

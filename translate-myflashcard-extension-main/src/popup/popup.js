...
async function translateText(text, targetLanguage, sourceLanguage = "auto") {
  ...
  if (text.length <= MAX_TEXT_LENGTH) {
    try {
      const url = `${GOOGLE_TRANSLATE_API}?client=gtx&sl=${sourceLanguage}&tl=${targetLanguage}&dt=t&q=${encodeURIComponent(
        text
      )}`;
  ...
  try {
    ...
    const translatedChunks = await Promise.all(
      chunks.map(async (chunk, index) => {
        ...
        const response = await fetch(url);
        if (!response.ok)
          throw new Error(`API không phản hồi cho phần ${index + 1}`);
        const data = await response.json();
...
async function detectLanguage(text) {
  try {
    ...
    const response = await fetch(
      `${GOOGLE_TRANSLATE_API}?client=gtx&sl=auto&tl=en&dt=t&q=${encodeURIComponent(
        sampleText
      )}`
    );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function performTranslation() {
    ...
    if (!text) {
      translatedText.ariaPlaceholder =
        "Chữ sau khi dịch sẽ được hiển thị ở đây...";
      return;
    }
    ...
    try {
      ...
      chrome.storage.local.set({
        lastWord: { text, result, sourceLanguage, targetLanguage },
      });
    } catch (error) {
  ...
  swapBtn.addEventListener("click", function () {
    ...
    if (sourceLanguageSelect.value !== "auto") {
      ...
      showNotification(
        "Không thể hoán đổi khi đang ở chế độ phát hiện tự động"
      );
    }
  });
  ...
  chrome.storage.local.get(["profile", "token"], async function (result) {
    if (result.token == null) {
      ...
      avatar.setAttribute(
        "href",
        "https://hustvanvo.io.vn/profile/" + result.profile._id
      );

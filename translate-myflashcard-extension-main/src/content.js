...
function createTranslationModal() {
  ...
  host.className = "myflashcard-translator-host";
  ...
  container.className = "myflashcard-translator";
...
function getHTMLTemplate(response) {
  ...
  const matches = translation?.matches || [
  ...
  return `
     <div class="quizzet-translator">
            ...
            <div class="header">
                <img src="${
                  ICONS.volume
                }" alt="Speak" class="speak-button" title="Phát âm thanh" />
                <img src="${
                  ICONS.sparkles
                }" title="Dịch bằng AI" alt="Speak" class="sparkles-button" />
                <div class="right">
                    <img src="${
                      ICONS.settings
                    }" alt="Settings" class="settings-button" />
                    <img src="${
                      ICONS.close
                    }" alt="Close" class="close-button" title="Đóng cửa sổ" />
                </div>
            </div>
            <main>
            ...
            <footer>
                  ...
                  profile
                    ? ` <div class="avatar  part-of-speech">
                            ...
                            <div class="name">
                                ...
                                <h2 class="">
                             ${
                               list_flashcard_id.language
                                 .slice(0, 3)
                                 .toUpperCase() +
                               " | " +
                               list_flashcard_id.title
                             }
                                </h2>
                            </div>
                        </div>`
                ...
                <button class="save" title="Thay đổi vị trí lưu flashcard, bấm vào setting">  <img src="${
                  ICONS.star
                }" alt="Save" class="save-button" /><p>Lưu vào Flashcard</p></button>
            </footer>
        </div>
...
async function setupModalEventListeners(text) {
  ...
  if (sparklesButton) {
    sparklesButton.addEventListener("click", function (e) {
      ...
      chrome.runtime.sendMessage(
        { action: "ai-enhance", text },
        function (response) {
          if (response.ok) {
            const p = (
              translationModal.shadowRoot || translationModal
            ).querySelector(".quizzet-translator main p.translate-vi");
  ...
  if (saveButton) {
    saveButton.addEventListener("click", function (e) {
      ...
      if (text) {
        ...
        chrome.storage.local.get(["list_flashcard_id"], function (result) {
          showNotification(
            `Đang lưu từ vựng vào ["${result.list_flashcard_id.title}"] `
          );
          list_flashcard_id = result.list_flashcard_id._id;
          if (textBtn) textBtn.textContent = `Đang lưu...`;
          if (!list_flashcard_id) {
            showNotification(
              "Vui lòng chọn flashcard trước khi lưu, vào setting để chọn flashcard"
            );
          ...
          chrome.runtime.sendMessage(
            ...
            function (response) {
              if (!response.ok) {
                ...
                showNotification(
                  `Lưu từ "${response.flashcard.title}" vào flashcard ["${result.list_flashcard_id.title}"] thành công`
                );
                saveButton.style.cursor = "pointer";
                saveButton.style.opacity = "1";
                if (textBtn) textBtn.textContent = `Lưu vào Flashcard`;

const QUIZZET_BACKEND_API = "https://api.hustvanvo.io.vn/api";
const QUIZZET_FRONTEND_API = "https://hustvanvo.io.vn/"; // Local development API endpoint
...
async function detectLanguage(text) {
  try {
    ...
    const url = `${GOOGLE_TRANSLATE_API}?client=gtx&sl=auto&tl=en&dt=t&q=${encodeURIComponent(
      sampleText
    )}`;
  ...
  if (text.length > MAX_TEXT_LENGTH) {
    try {
      ...
      const translations = await Promise.all(
        chunks.map((chunk) =>
          translateChunk(chunk, targetLanguage, sourceLanguage)
        )
      );

      // Combine main translations
      const mainTranslation = translations
        .map((t) => t.mainTranslation)
        .join("");
...
async function translateChunk(text, targetLanguage, sourceLanguage) {
  try {
    const url = `${GOOGLE_TRANSLATE_API}?client=gtx&sl=${sourceLanguage}&tl=${targetLanguage}&dt=t&dt=bd&dt=at&dt=md&dt=rm&dt=ss&q=${encodeURIComponent(
      text
    )}`;
    const response = await fetch(url);
    if (!response.ok) {
      ...
      data[1].forEach((wordGroup) => {
        ...
        if (
          ...
          if (translationsByType[partOfSpeech].length > 5) {
            translationsByType[partOfSpeech] = translationsByType[
              partOfSpeech
            ].slice(0, 5);
          }
        }
  ...
  if (request.action === "translate") {
    ...
    translateText(text, targetLanguage, sourceLanguage)
      .then((translationResult) => {
        ...
        for (const [partOfSpeech, translations] of Object.entries(
          translationResult.translationsByType
        )) {
          matches.push({
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "save-translation") {
    chrome.storage.local.get(["token", "list_flashcard_id"], (result) => {
      const optimizedPrompt = OptimizedPrompt(
        request.text,
        result.list_flashcard_id.language
      );
      fetch(`${QUIZZET_BACKEND_API}/flashcards/create-ai`, {
        ...
        body: JSON.stringify({
          ...
          language: result.list_flashcard_id.language,
        }),
      })
  ...
  if (request.action === "ai-enhance") {
    chrome.storage.local.get(["token", "target_language"], (result) => {
      const optimizedPrompt = optimizePronptAIEnhance(
        request.text,
...
function fetchTokens() {
  return new Promise((resolve, reject) => {
    chrome.cookies.get(
      { url: QUIZZET_FRONTEND_API, name: "token" },
      (cookie) => {
        if (cookie) {
              ...
              if (data.ok) {
                chrome.storage.local.get(["list_flashcard_id"], (result) => {
                  if (result.list_flashcard_id) {
                    ...
                    chrome.storage.local.set({
                      list_flashcard_id: data.listFlashCards[0],
                    });
                  }
                  chrome.storage.local.set({ profile: data.user });
                  chrome.storage.local.set({
                    listFlashCards: data.listFlashCards,
                  });
...
function checkUpdate() {
  fetch(
    "https://raw.githubusercontent.com/huytd2004/CNWeb_AIflashcard/main/translate-myflashcard-extension-main/data/version.json"
  )
    .then((response) => response.json())
    .then((data) => {
      ...
      if (data.version > currentVersion) {
        ...
        chrome.notifications.onClicked.addListener((notificationId) => {
          if (notificationId === "update_notification") {
            chrome.tabs.create({
              url: "https://github.com/huytd2004/CNWeb_AIflashcard/tree/main/translate-myflashcard-extension-main",
            });
          }
        });
      }
